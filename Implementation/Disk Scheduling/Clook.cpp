...
int main(){
    ...
    int head=queue[0];                                  
    int ScanQueue[2*n];                  //as we have include 0 and 199 also the size will not be n so we dont know how many times they come so we used size as n*n                                    
   ...
   cout<<"\nEnter Direction: "<<endl;
    ...
    if(d==-1){                           //Left
       ...
       // ScanQueue[k]=0;                   //In scan we touch the extreme as well
        ...
        for(i=pos+1;i<n;i++){             //moves from next element from head in sorted queue towards right now
