...
int main(){
    int q[4]={-1,-1,-1,-1};
    int ref[16]={7,0,1,2,0,3,0,4,2,3,0,3,2,1,0,0};
    int N=16;
    int r=0;                            //you wull get to know the use of these initializations by reading further...
    int elpos=0; 
    int qelpos;
    int flag=0;
    for(int i=0;i<N;i++){                    //main loop: it traverses ref string if q dosent have that element it replaces the el in q that is most left in ref queue.
        ...
        if(flag!=1){                        //runs if the el in ref queue is not in q queue
            ...
            {   elpos=0;
                for(int lt=0;lt<4;lt++){                         //checks ref queue from that element towards left
                    for(int l=i;l<N;l++){                   //compares each left el of ref queue with the elements aready present in q queue
                        if(ref[l]==q[lt] && l>elpos){          //if the el we are at due to 24 line loop is in queue and its index is 'least from 0 index of ref queue' (at max from the element(but first occurance from that el in ref queue) we have to fit in) then save the index
                            ...
                            qelpos=lt;  
                            break;                 //stores its index in q queue
                        }
                        else{
                             int count=0;
